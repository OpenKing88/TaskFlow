...
import com.openking.oklib.executor.OKExecutor
import com.openking.oklib.utils.MainHandler
 ...
 * date: 2021/5/2117:36
 * des: taskFlow运行时的任务调度器
 * 1。根据task的属性用不同的策略（线程、同步、延迟）来调度任务
 * 2。校验依赖树中是否存在环形依赖
...
internal object TaskRuntime {
    ...
    fun executeTask(task: Task) {
        if (task.isAsyncTask) {
            OKExecutor.execute(runnable = task)
        } else {
            //else 里面的 都是在主线程 执行的
